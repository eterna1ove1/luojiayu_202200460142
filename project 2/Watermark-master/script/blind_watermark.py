...
class BlindWatermark():
    ...
    def _gene_signature(wm, size, key):
        ...
        wU, _, wV = np.linalg.svd(np.asmatrix(wm))  # 修改点1：np.mat→np.asmatrix
        ...
        sqrts = int(np.sqrt(size))  # 修改点2：直接使用Python内置int
    ...
    def calc_sim(sig1, sig2s):
        ...
        for sig2 in sig2s:
            match_cnt = np.sum(
                np.equal(np.array(sig1, dtype=np.int32), np.array(sig2, dtype=np.int32)))  # 修改点3：np.int→np.int32
...
class DCT_watermark(BlindWatermark):
    ...
    def inner_embed(self, B, signature):
        sig_size = int(np.sqrt(len(signature)))  # 修改点4：直接使用Python内置int
        size = self.size
    ...
    def inner_extract(self, B, signature):
        sig_size = int(np.sqrt(len(signature)))  # 修改点5：np.int→int
        size = self.size
        ...
        for x, y in embed_pos:
            ext_sig = np.zeros(len(signature), dtype=np.int32)  # 修改点6：np.int→np.int32
