...
static uint32_t T_table[4][T_Table_SIZE]; // 为每个字节分配一个 4-元组的 T-Table
...
void generate_t_table() {
    for (int i = 0; i < 256; ++i) {
        ...
        t = L(t);  // L 变换
        // 将结果存储在 T-Table 中
        T_table[0][i] = (t & 0xFF000000) >> 24;
        T_table[1][i] = (t & 0x00FF0000) >> 16;
        T_table[2][i] = (t & 0x0000FF00) >> 8;
        T_table[3][i] = (t & 0x000000FF);
    }
}
...
void sm4_encrypt_ttable(const uint8_t input[16], uint8_t output[16], const uint32_t rk[32]) {
    ...
    for (int i = 0; i < 32; i++) {
        ...
        X[i+4] = X[i] ^ 
                T_table[0][(temp >> 24) & 0xFF] ^
                T_table[1][(temp >> 16) & 0xFF] ^
                T_table[2][(temp >>  8) & 0xFF] ^
                T_table[3][ temp        & 0xFF];
    }
...
int main() {
    ...
    for (int i = 0; i < TEST_ROUNDS; ++i) {
        for (int j = 0; j < TEST_GROUPS; ++j) {
            sm4_encrypt_ttable(plaintexts[j], ciphertexts[j], enc_rk);
        }
    }
    end = __rdtsc();
    _ReadWriteBarrier();
