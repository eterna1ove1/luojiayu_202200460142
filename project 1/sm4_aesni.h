...
// 加密4个SM4分组(并行处理)
static inline void sm4_encrypt_4blocks_aesni(const uint8_t input[64], uint8_t output[64], const uint32_t rk[32]) {
    __m128i X[4], Tmp[4];
...
// 解密4个SM4分组(并行处理)
static inline void sm4_decrypt_4blocks_aesni(const uint8_t input[64], uint8_t output[64], const uint32_t rk[32]) {
    __m128i X[4], Tmp[4];
    __m128i vindex = _mm_setr_epi8(3, 2, 1, 0, 7, 6, 5, 4, 11, 10, 9, 8, 15, 14, 13, 12);
